...
const wchar_t * irrlight::Terminal::text() const{
	return ((irr::gui::IGUIEditBox *)d->getGUIEnvironment()->getRootGUIElement()->getElementFromId(COMMAND_BOX, true))->getText();
}

void irrlight::Terminal::text(const wchar_t* text){
	((irr::gui::IGUIEditBox *)d->getGUIEnvironment()->getRootGUIElement()->getElementFromId(COMMAND_BOX, true))->setText(text);
}
